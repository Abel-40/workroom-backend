...
class TaskUpdateIn(Schema):
    ...
    estimated_time_hours: float | None = Field(default=None, gt=0, le=1000)
    spent_time_hours: float | None = Field(default=None, ge=0, le=1000)
...
def task_data(task: Task) -> dict:
    return {
        ...
        'spent_time_hours': task.spent_time.total_seconds() / 3600 if task.spent_time else None,
...
async def list_tasks(request, project_id: UUID, status: StatusLiteral | None = None, page: int = 1, page_size: int = DEFAULT_PAGE_SIZE):
    ...
    return payload('Tasks retrieved successfully.', 200, True, {
        'results': [task_data(task) for task in items], 'meta': meta,
    })
...
async def get_task(request, task_id: UUID):
    ...
    return payload('Task retrieved successfully.', 200, True, {'task': task_data(task)})
...
async def update_task(request, task_id: UUID, data: TaskUpdateIn):
    ...
    if 'spent_time_hours' in updates:
        updates['spent_time'] = _hours_to_duration(updates.pop('spent_time_hours'))
