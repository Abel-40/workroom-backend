...
| D8 | ~~Anyone who can view a project can read — and a project manager can delete — another user's AI Assistant conversation~~ | **FIXED in WP12.** Queries are private to `requested_by`, with no project-manager or company-owner override in either direction; an unreadable one answers 404 rather than 403. The listing filters in the query. Deletion is owner-only as a *removed code path* rather than a tightened check — `delete_assistant_query` takes no argument that permits deleting another's. The owner may share one answer with project VIEW and revoke it. |
